...
#         self.local_fc_1 = nn.Linear(source_embed_dim, source_embed_dim)
#         self.local_fc_2 = nn.Linear(source_embed_dim, 1)
    ...
    def forward(self, input, source_hids, encoder_padding_mask, t):
        ...
        if m_len < 2 * self.window_size + 1:
            ...
            attn_scores = F.softmax(attn_scores, dim=0)  # srclen x bsz
            # pt = local_p(input, self.local_fc_1, self.local_fc_2, m_len)
            # pt = torch.round(pt)
#             pt = local_m(input, t)
#             pt = self.window_size + pt
            ...
            x_ = x.unsqueeze(2)  # 2,16,1
            x_ = x_.permute(0, 2, 1)
            pt = self.tanh(self.fc1(x_))
            pt = self.sigmoid(self.fc2(pt))
            pt = pt.view(batch_size, 1)
            pt = self.window_size + lengths.float() * pt

            # print(attn_scores)
            attn_scores = local_score(attn_scores.unsqueeze(2), pt, m_len, self.window_size / 2, encoder_padding_mask)


        # sum weighted sources
        # attn_scores: max_len x batch_size
...
def local_score(attention_score, pt, seq_len, sigma, padding_mask):
    # print(attention_score.shape)
    attention_score = attention_score.transpose(1, 0)
    # print(attention_score.shape)
    # attention_score: max_len x batch_size x 1
    # print("-----------------------------")
    # print(pt.shape)
    # print(seq_len)
    len = torch.tensor(seq_len).repeat(pt.size()[0], 1).cuda()
    t_sigma = torch.tensor(sigma).repeat(pt.size()[0], 1).cuda()
    # print(seq_len)
    # print(pt.dtype)
    # print(len.dtype)
    # print(pt.shape)

    # pt = torch.where(pt < 2 * t_sigma, 2 * t_sigma, pt)
    # pt = torch.where(pt + 2 * sigma > len, len - 2 * sigma, pt)
    pt = torch.where(pt < 2 * t_sigma, 2 * t_sigma, pt)
    pt = torch.where(pt + 2 * sigma > len-1, len-1 - 2 * sigma, pt)
    # print(pt.shape)

    pt = pt.unsqueeze(2)

    s = torch.arange(0, seq_len).cuda()
    s = s.view([1, seq_len, 1]).repeat([attention_score.size()[0], 1, 1])

    # print(s)
    # mask = torch.where((s > pt - 2 * sigma) & (s < pt + 2 * sigma), torch.ones_like(s), torch.zeros_like(s))
    mask = torch.where((s >= pt - 2 * sigma) & (s <= pt + 2 * sigma), torch.ones_like(s), torch.zeros_like(s))

    attention_score = attention_score * torch.exp(-(s - pt) ** 2 / (2 * sigma ** 2)) * mask
    attention_score = attention_score / torch.sum(attention_score, dim=1, keepdim=True)

    attention_score = attention_score.squeeze(2).transpose(0, 1)
    
    epsilon = 0
    if padding_mask is not None:
        attention_score = (
            attention_score.float()
                .masked_fill_(padding_mask, float(0))
                .type_as(attention_score)
        )  # FP16 support: cast to float and back
    ...
    def __init__(
        ...
        if attention:
            ...
            self.attention = AttentionLayer(
                hidden_size, encoder_output_units, hidden_size, window_size, bias=False
            )
        else:
            self.attention = None
        ...
        for j in range(seqlen):
            ...
            if self.attention is not None:
                ...
                out, attn_scores[:, j, :] = self.attention(
                    hidden, encoder_outs, encoder_padding_mask, j
                )
            else:
                out = hidden
...
@register_model_architecture("lstm_sps", "lstm_sps")
...
@register_model_architecture("lstm_sps", "lstm_sps_2l")
def base_architecture(args):
    args.dropout = getattr(args, "dropout", 0.1)
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 512)
    args.encoder_embed_path = getattr(args, "encoder_embed_path", None)

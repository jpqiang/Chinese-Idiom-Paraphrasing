...
TEXT=data/in_domain
DATADIR=data-bin/in_domain
mkdir -p $DATADIR

# tokenize dataset if need
# python ./preprocess/idoms_process.py
...
$SUBWORD_NMT/apply_bpe.py -c $TEXT/${CODES}.src --vocabulary $TEXT/${VOCAB}.src < $TEXT/valid.src > $TEXT/valid.${NUM_OPS}.bpe.src
$SUBWORD_NMT/apply_bpe.py -c $TEXT/${CODES}.dst --vocabulary $TEXT/${VOCAB}.dst < $TEXT/valid.dst > $TEXT/valid.${NUM_OPS}.bpe.dst

# encode test
echo "Applying vocab to test"
$SUBWORD_NMT/apply_bpe.py --num-workers 8 -c $TEXT/${CODES}.src --vocabulary $TEXT/${VOCAB}.src < $TEXT/test.src > $TEXT/test.${NUM_OPS}.bpe.src
$SUBWORD_NMT/apply_bpe.py --num-workers 8 -c $TEXT/${CODES}.dst --vocabulary $TEXT/${VOCAB}.dst < $TEXT/test.dst > $TEXT/test.${NUM_OPS}.bpe.dst
